...
def secrets_engine_enabled(name, engine_type, description='', mount_point=None,
    ...
    :param engine_type: The type of secrets engine to enable (e.g. MySQL)
    :param description: The description to set for the enabled engine
    :param mount_point: The root path for the engine
    :param connection_config_path: The full path to the endpoint used for
                                   configuring the connection (needed for
                                   e.g. Consul)
    :param connection_config: The configuration settings for the engine
                              connection
    ...
    :param lease_default: The default allowed lease for credentials retrieved from
                          the engine
    :param ttl_max: The maximum TTL for a lease generated by the engine. Uses
                    the mounts/<mount_point>/tune endpoint.
    :param ttl_default: The default TTL for a lease generated by the engine.
    ...
    """
    engines = __salt__['mdl_vault.list_secrets_engines']().get('data', {})
    ...
    for path, settings in engines.items():
        if path.strip('/') == mount_point and settings['type'] == engine_type:
            engine_enabled = True

    if engine_enabled and not override:
        ret['comment'] = ('The {secret_type} secrets engine mounted at /{mount} is '
            'already enabled.'.format(secret_type=engine_type, mount=mount_point))
        ret['result'] = True
    elif __opts__['test']:
        ret['result'] = None
    else:
        ...
        if connection_config:
            ...
            try:
                ...
                ret['comment'] += ('The secrets engine was enabled but the connection '
        ...
        if lease_max or lease_default:
            ...
            try:
                ...
                ret['comment'] += ('The secrets engine was enabled but the lease '
                                  'length could not be configured\n'.format(e))
                log.exception(e)
                raise salt.exceptions.CommandExecutionError(str(e))
        ret['comment'] += ('The {engine} secrets engine has been successfully mounted '
                          'at /{mount}.'.format(engine=engine_type,

...
# Based on https://github.com/mitodl/vault-formula/blob/master/_utils/vault.py (BSD 3-clause)
...
class VaultClient(object):
    ...
    def configure_auth_backend(self, mount_point, config):
        """
        POST /auth/<mount point>/config
        """
        self._post('/v1/auth/{0}/config'.format(mount_point), json=config)

    def get_auth_backend_config(self, mount_point):
        """
        GET /auth/<mount point>/config
        """
    ...
    def __request(self, method, url, headers=None, **kwargs):
        ...
        if response.status_code >= 400 and response.status_code < 600:
            ...
            message = 'Minion got vault error trying %s %s: %s (%s)' % (
                method.upper(), url, response.status_code, text)

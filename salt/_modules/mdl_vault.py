...
def initialize(secret_shares=5, secret_threshold=3, pgp_keys=None,
    ...
    if keybase_users and isinstance(keybase_users, list):
        ...
        for user in keybase_users:
            log.debug('Retrieving public keys for Keybase user {}.'
                      .format(user))
            keybase_keys.append(__utils__['mdl_vault.get_keybase_pubkey'](user))
    ...
    client = __utils__['mdl_vault.build_client']()
    try:
        ...
        if unseal:
            __utils__['mdl_vault.wait_after_init'](client)
            log.debug('Unsealing Vault with generated sealing keys.')
            __utils__['mdl_vault.unseal'](sealing_keys)
    ...
    try:
        if pgp_keys and unseal:
            ...
            __utils__['mdl_vault.rekey'](secret_shares, secret_threshold,
...
def clean_expired_leases(prefix='', time_horizon=0):
    ...
    for index, lease in enumerate(expired_leases):
        ...
        except __utils__['mdl_vault.vault_error']():
...
def check_cached_lease(path, cache_prefix='', **kwargs):
    ...
    renewal_threshold = __opts__.get('vault.lease_renewal_threshold',
                                    {'days': 7})
    vault_client = __utils__['mdl_vault.build_client']()
...
def _register_functions():
    ...
    for method_name in dir(__utils__['mdl_vault.vault_client']()):
        if not method_name.startswith('_'):
            method = getattr(__utils__['mdl_vault.vault_client'](), method_name)
            if not isinstance(method, property):
                if method_name == 'list':
                    method_name = 'list_values'
                globals()[method_name] = __utils__['mdl_vault.bind_client'](method)
